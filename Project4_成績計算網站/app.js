...
time_line
  ...
  .fromTo(animation, 0.3, { opacity: 1 }, { opacity: 0 }); // 動畫結束後視窗消失，顯示後面的介面

setTimeout(() => {
  (animation.style.pointerEvents = "none"), 2500;
});
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    let class_name = graders[i].children[0].value.trim();
    let class_number = graders[i].children[1].value.trim();
    let class_credit = graders[i].children[2].value.trim();
    let class_grade = graders[i].children[3].value.trim();

    if (class_name && class_number && class_credit && class_grade) {
      let class_object = {
        class_name,
        class_number,
        class_credit: Number(class_credit),
        class_grade,
      };
  ...
  // SELECT可直接用JS更改
  greders = document.querySelectorAll("div.grader");
  for (let i = 0; i < greders.length; i++) {
    greders[i].children[3].value = objectArray[i].class_grade;
  }
